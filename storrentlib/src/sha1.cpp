...
namespace storrent
{
    ...
    void sha1::set_final(std::string final) { _result_str = std::move(final); }

    void sha1::reset_digest() { reset(digest, buffer, transforms); }
    ...
    std::string sha1::from_file(const std::wstring& filename)
    {
        std::ifstream stream(filename.c_str(), std::ios::binary);
        sha1 checksum;
        checksum.update(stream);
    ...
    std::string sha1::from_str(const std::string& msg_str)
    {
        sha1 checksum;
        checksum.update(msg_str);
        return checksum.final();
    }

    std::string sha1::from_str(const std::wstring& msg_str)
    {
        sha1 checksum;
        checksum.update((char*)msg_str.data());
        return checksum.final();
    }
}
