        ...
        if (bencode_doc->state() == bencode_parse_state::parsed)
        {
            ...
            if (auto info_entry = main_dict["info"]; info_entry != nullptr)
            {
                ...
                else
                {
                    ...
                    torrent_files.emplace_back(
                        /*std::move(torrent_file {*/ std::vector<std::string>{name},
                        size_t(info_dict["length"]->int_val()) /*})*/
                    );
                }

                std::string comment = main_dict.find("comment") != main_dict.cend() ? main_dict["comment"]->str_val()
                                                                                    : "";
                std::string created_by = main_dict.find("created by") != main_dict.cend()
                                             ? main_dict["created by"]->str_val()
                                             : "";
                intptr_t creation = main_dict.find("creation date") != main_dict.cend()
                                        ? main_dict["creation date"]->int_val()
                                        : 0;

                // auto announces = std::vector<std::string>{main_dict["announce"]->str_val()};
                std::string announce_url = main_dict["announce"]->str_val();
                ...
                return std::make_shared<torrent>(filename,
                                                 name,
                                                 torrent_files,
                                                 pieces,
                                                 piecelen,
