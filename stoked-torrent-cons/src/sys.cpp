...
namespace stoked::btp::utils {
    ...
    std::string module_info::get_file_name_from_module(HMODULE aModuleHandle) {
        ...
        do {
            ...
            copied = GetModuleFileNameEx(GetCurrentProcess(), aModuleHandle, static_cast<LPSTR>(filename.get()), (DWORD)strlen(filename.get()));
            if (!copied) {                                
                return "";
            }
    ...
    void module_info::set_val(file_version_info_value aValue) {
        _file_version_info_val[static_cast<size_t>(aValue)] = module_info::get_file_version_info_value(_filename, aValue);
    }
    ...
    const std::string& module_info::module_filename(void) const {
        return _filename;
    }

    void module_info::set_refresh(size_t aRefreshIdx) {
        _refresh[aRefreshIdx] = true;
    }
    ...
    const std::string& module_info::comments(void) {
        return this->get_val(file_version_info_value::comments);
    }

    const std::string& module_info::company_name(void) {
        return this->get_val(file_version_info_value::company_name);
    }
    ...
    const std::string& module_info::file_version(void) {
        return this->get_val(file_version_info_value::file_version);
    }
    ...
    const std::string& module_info::product_name(void) {
        return this->get_val(file_version_info_value::product_name);
    }
...
#pragma region PERFORMANCE TIMER
    ...
    inline
#endif			 
        void performance_timer::reset(void) {
        _stop = _start = 0;
        _running = false;
    }
    ...
    inline
#endif			 
        void performance_timer::start(void) {
        _running = true;
        _stop = 0;
        _start = counter();
    }
    ...
    inline
#endif			 
        void performance_timer::stop(void) {
        _stop = counter();
        _running = false;
    }
    ...
    inline
#endif			 
        double performance_timer::elapsed_ms(void) const {
        if (_running) {
    ...
    inline
#endif			 
        long long performance_timer::counter(void) {
        LARGE_INTEGER zLI;
    ...
    inline
#endif			 
        double performance_timer::elapsed_ms(long long aStart, long long aStop) const {
        _ASSERTE(aStart >= 0);
        _ASSERTE(aStop >= 0);
    ...
    inline
#endif			 
        long long performance_timer::frequency(void) {
        LARGE_INTEGER zLI;
...
#pragma endregion	
    ...
    double get_cpu_used_by_process(void) {
        ...
        percent = (double)(sys.QuadPart - zLastSysCPU.QuadPart) +
            (user.QuadPart - zLastUserCPU.QuadPart);
