...
namespace stoked {
    ...
    public:
        ...
        explicit constexpr bencode_element(bencode_element_type type)
            : _type{type} {
        }
        ...
        virtual bencode_element_type type() const {
            return _type;
        }
    private:
    ...
    public:
        ...
        bencode_string(std::string val)
            : bencode_element(bencode_element_type::string),
            _val{std::move(val)} {}
        ...
        virtual intptr_t int_val() const override {
            intptr_t result = 0;
            if (auto convert_result = std::from_chars(_val.c_str(), _val.c_str() + _val.length(), result); convert_result.ec == std::errc()) {
                return result;
            } else {
    ...
    public:
        ...
        bencode_integer(intptr_t val)
            : bencode_element(bencode_element_type::integer),
            _val{val} {}
        ...
        virtual std::string encoded_val() const  override {
            return std::string("i") + std::to_string(_val).append("e");
        }
    ...
    public:
        ...
        const bencode_element* const operator[](size_t idx) {
            return _val[idx].get();
        }

        void append_val(bencode_element* element) {
            _val.push_back(std::shared_ptr<bencode_element>(element));
        }

        void append_val(std::shared_ptr <bencode_element> element) {
            _val.push_back(std::move(element));
        }

        std::vector<std::shared_ptr<bencode_element>>& value() {
            return _val;
        }

        const std::vector<std::shared_ptr<bencode_element>>& value() const {
            return _val;
        }
    ...
    public:
        ...
        std::map<std::string, std::shared_ptr<bencode_element>>& value() {
            return _val;
        }

        const std::map<std::string, std::shared_ptr<bencode_element>>& value() const {
            return _val;
        }
        ...
        virtual std::string encoded_val() const override {
            ...
            for (const auto& entry : _val) {
                result.append(std::to_string(entry.first.length()).append(":").append(entry.first).append(entry.second->encoded_val()));
            }
            return result.append("e");
        }
    ...
    public:
        ...
        bencode_doc()
            : _filename{},
            _state{bencode_parse_state::unk},
            _parse_err{},
            _elements{} {}
        ...
        std::string parse_err() const {
            return _parse_err;
        }

        const bencode_element& get_element(size_t idx) const {
            return *_elements.at(idx).get();
        }

        bencode_parse_state state() const {
            return _state;
        }
    ...
    private:
        static std::shared_ptr<bencode_element> read_element(const std::string& content, size_t& idx, std::string& err_msg) {
            switch (content[idx]) {
                case '0':
                case '1':
                case '2':
        ...
        static std::shared_ptr<bencode_element> read_string(const std::string& content, size_t& idx, std::string& err_msg) {
            if (size_t pos = content.find(':', idx); pos != std::string::npos) {
                size_t parsed_len = 0;

                if (auto parsed_result = std::from_chars(content.c_str() + idx, content.c_str() + pos, parsed_len); parsed_result.ec == std::errc()) {
        ...
        static std::shared_ptr<bencode_element> read_integer(const std::string& content, size_t& idx, std::string& err_msg) {
            idx++;
            if (size_t pos = content.find('e', idx); pos != std::string::npos) {
                intptr_t val = -1;
                if (auto parsed_result = std::from_chars(content.c_str() + idx, content.c_str() + pos, val); parsed_result.ec == std::errc()) {
                    idx = pos + 1;
        ...
        static std::shared_ptr<bencode_element> read_dictionary(const std::string& content, size_t& idx, std::string& err_msg) {
